...
This is an MCP (Model Context Protocol) server that exposes Gmail inbox tools via Streamable HTTP transport. It implements two-layer OAuth: MCP-level JWT authentication and Google OAuth for Gmail access.
...
- **`src/mcp/server.ts`** - MCP server with all 23 tools registered; validates inputs with Zod schemas
...
- **`src/store/sqlite.ts`** - SQLite token store; refresh tokens encrypted with AES-256-GCM
